...
優先度 2 : 宛先MACアドレスがマルチキャストMACアドレス('01:00:5e:XX:XX:XX')であればDropする … エントリD (Drop)
優先度 1 : GoTo Fowardingテーブル (Table ID: 1) … エントリG (GoTo)
```

```
【Fowardingテーブル (Table ID: 1)】
優先度 1 : コントローラにPortOutし，パケットをFloodingする … エントリP (PacketIn)
```

### PacketInが発生したとき
PacketInが発生すると`packet_in`ハンドラが呼び出される．  
そのとき，Fowardingテーブルに以下のエントリを追加する．

```
優先度: 1
エントリ破棄時間: 180秒 (AGING_TIME)
マッチング: 送信先MAC が パケットの送信元MAC
...
2. Filteringテーブルで送信先(**host2**)のMACアドレスが`00:00:00:00:00:02`なので，**エントリD**が適用されない為，パケットがDropされずに**エントリG**が適用される．  
3. Fowardingテーブルで**エントリP**が適用され，コントローラにPacketInを送信し，パケットをフラッディングする．  
4. 【コントローラ】PacketInを受信したコントローラは，宛先のMACアドレスが**host1**(`00:00:00:00:00:01`)のパケットの転送先を**host1**のポート番号(ポート1)に転送するフローエントリをFowardingテーブルに登録する．
...
```
【Fowardingテーブル (Table ID: 1)】
優先度 2 : 宛先MACアドレスがhost1(`00:00:00:00:00:01`)であれば，パケットをポート1にPortOutする． … エントリ1 (host1)
優先度 1 : コントローラにPortOutし，パケットをFloodingする … エントリP (PacketIn)
```
...
2. Filteringテーブルで送信先(**host2**)のMACアドレスが`00:00:00:00:00:02`なので，**エントリD**が適用されない為，パケットがDropされずに**エントリG**が適用される．  
3. Fowardingテーブルで**エントリ1**が適用され，パケットをポート1(**host1**)にPortOutする．
...
上から順に**エントリD**，**エントリG**，**エントリP**が設定されていることが確認できる．
...
新たに追加されたフローエントリは宛先MACアドレスが`00:00:00:00:00:01`のパケットをポート1(**host1**)に転送するといったフローエントリになっている．
...
また，前述のとおりフローテーブル(`Fowardingテーブル`)にフローエントリが追加されていないことが確認できる．
...
今回の課題で利用したコントローラは，パケットが送信され**PacketInが発生したとき**にパケットの送信元MACアドレスと送信元ポート番号からフローテーブルにフローエントリを追加している．
